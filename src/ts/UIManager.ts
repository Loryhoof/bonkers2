...
const positionText = document.getElementById("positionText") as HTMLElement;
const hotbarItems = document.querySelectorAll(".hotBarItem") as
  | HTMLElement
  | any;
const healthSlider = document.getElementById("healthSlider") as HTMLElement;

const networkIdText = document.getElementById("networkIdText") as HTMLElement;
...
const pingText = document.getElementById("pingText") as HTMLElement;
...
export default class UIManager {
  ...
  updatePositionText(position: THREE.Vector3) {
    positionText.innerHTML = `X: ${Math.floor(position.x)}, Y: ${Math.floor(
      position.y
  ...
  setNetworkIdText(text: string) {
    networkIdText.innerHTML = text;
  }
  ...
  updatePing(text: string) {
    pingText.innerHTML = `Ping: ${text} ms`;
  }
