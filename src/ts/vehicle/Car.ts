...
export default class Car implements Vehicle {
    ...
    init() {
        ...
        const rbDesc = (window as any).RAPIER.RigidBodyDesc.dynamic().setTranslation(this.vehicleModel.position.x, this.vehicleModel.position.y, this.vehicleModel.position.z)
        .setAdditionalMass(1000)
        .setLinearDamping(0.5)

        const rigidBody = PhysicsManager.getInstance().physicsWorld.createRigidBody(rbDesc);

        const colDesc = (window as any).RAPIER.ColliderDesc.cuboid(scale.x / 2, scale.y / 2, scale.z / 2);
        const collider = PhysicsManager.getInstance().physicsWorld.createCollider(colDesc, rigidBody);
        ...
        for (let i = 0; i < wheelPositions.length; i++) {
            ...
            this.wheels[i].position.copy(this.vehicleModel.position).add(wheelPositions[i])
    ...
    updateWheels(elapsedTime: number, deltaTime: number) {
        ...
        for (let i = 0; i < this.wheels.length; i++) {
            ...
            let wheelPos = this.vehicleModel.position.clone().add(this.wheels[i].position)

            //

            let {ray, hit} = PhysicsManager.getInstance().raycastFull(wheelPos, new THREE.Vector3(0, -1, 0), this.physicsObject.rigidBody)
            ...
            if(hit !== null) {
                ...
                if(distance <= (this.maxLength + this.wheelRadius)) {
                    ...
                    this.springVelocity = ((this.lastLength - this.springLength) / deltaTime / 1000)

                    this.springLength = THREE.MathUtils.clamp(this.springLength, this.minLength, this.maxLength)
                    ...
                    this.suspensionForce = UP.clone().applyQuaternion(this.vehicleModel.quaternion).multiplyScalar(lala)
                    //console.log(lala)
                    this.physicsObject.rigidBody.addForceAtPoint(this.suspensionForce, hitPoint, true) // { x: 0.0, y: 1000.0, z: 0.0 }, { x: 1.0, y: 2.0, z: 3.0 }
                    this.physicsObject.rigidBody.resetForces(true)
            ...
            this.wheels[i].position.x = this.vehicleModel.position.clone().add(wheelPositions[i]).x
            this.wheels[i].position.z = this.vehicleModel.position.clone().add(wheelPositions[i]).z
