...
import IKJoint from './IKJoint.ts';
...
class IKChain {
  ...
  public isIKChain: boolean
  public totalLengths: number
  public base: any
  public effector: any
  public effectorIndex: any
  public chains: any

  public origin: THREE.Vector3 | null
  ...
  add(joint: IKJoint, { target } : any = {}) {
    if (this.effector) {
      throw new Error('Cannot add additional joints to a chain with an end effector.');
    }

    if (!joint.isIKJoint) {
      if (joint.bone.isBone) {
        joint = new IKJoint(joint.bone);
      } else {
