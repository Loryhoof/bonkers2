...
let ray = new (window as any).RAPIER.Ray(
  new (window as any).RAPIER.Vector3(0, 0, 0),
  new (window as any).RAPIER.Vector3(0, 0, 0)
);
...
export default class PhysicsManager {
  ...
  createFixedBox(
    ...
    const rbDesc = (window as any).RAPIER.RigidBodyDesc.fixed()
  ...
  createPlayerCapsule(): PhysicsObject {
    let rbDesc = (window as any).RAPIER.RigidBodyDesc.dynamic()
    ...
    let capsuleColDesc = (window as any).RAPIER.ColliderDesc.capsule(
      halfHeight,
      radius
    );
