...
export default class Rifle extends THREE.Object3D implements Firearm {
  ...
  shoot() {
    if (this.ammo > 0 && !this.isReloading) {
      ...
      const objectsToIntersect = this.scene.children.filter(
        (object) =>
          object !== this.model &&
          object !== this.muzzleFlash.mesh &&
          !object.userData.ignoreRayHit
      ); // && !object.ignoreRayHit
      ...
      if (intersects.length > 0) {
        ...
        const recoilVector = new THREE.Vector3(0, 0, 1)
          .clone()
          .multiplyScalar(0.05);
        this.model?.position.add(recoilVector);

        const initialSlidePosition = this.model_slide?.position.clone();
        this.model_slide?.position.add(
          new THREE.Vector3(0, 0, 1).clone().multiplyScalar(0.02)
        );
  ...
  setActive(bool: boolean, owner: Player) {
    ...
    this.recoil.applySettings(20, 15);
  }
  ...
  update(elapsedTime: number, deltaTime: number) {
    ...
    let newFov = THREE.MathUtils.lerp(
      this.camera.fov,
      cameraTargetFov,
      lerpFactor
    );
    ...
    if (
      (this.isAds && this.owner.controller.isWalking) ||
      (this.isAds && this.owner.controller.isSprinting)
    ) {
      frequency = 1;
      amplitude = 0.005;
    }
